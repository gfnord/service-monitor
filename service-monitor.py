...
def usage():
    ...
    print('\tlabel	  \tlabel for the service')
...
def send_error(test_type, server_info, label, email_address):
    subject = '[%s] %s: %s %s error' % (label, asctime(), test_type.upper(), server_info)
    message = '[%s] There was an error while executing a %s test against %s.' % (label, test_type.upper(), server_info)
...
if __name__ == '__main__':
    if len(argv) != 5:
        ...
        send_error(argv[1], argv[2], argv[3], argv[4])
